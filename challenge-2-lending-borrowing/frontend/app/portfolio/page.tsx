...
export default function PortfolioPage() {
	...
	const { data: portfolioData } = useReadContracts({
		contracts: [
			...
			{
				address: contractAddresses.LendingToken as Address,
				abi: erc20Abi,
				functionName: 'decimals',
			},
			{
				address: contractAddresses.CollateralToken as Address,
				abi: erc20Abi,
				functionName: 'decimals',
			},
			{
	...
	const loanDetails = portfolioData?.[1]?.result as
	...
	const healthFactor =
		hasActiveLoan && loanAmount > 0
			? (lockedCollateral * BigInt(100)) /
			  (loanAmount * (collateralFactor || BigInt(150)))
			: BigInt(0);
	...
	const getHealthFactorStatus = () => {
		...
		if (healthFactor >= BigInt(150))
			return {
				status: 'Safe',
				color: 'text-green-600',
				variant: 'default' as const,
			};
		if (healthFactor >= BigInt(120))
			return {
				status: 'Moderate',
	...
	const totalCollateralValue = depositedCollateral
		? Number(formatUnits(depositedCollateral, collateralDecimals || 18))
		: 0;
	const totalLoanValue = hasActiveLoan
		? Number(formatUnits(loanAmount, lendingDecimals || 18))
		: 0;
