...
export function TokenSelector({
	...
	const {
		...
		isLoading: isSearchTokenDataLoading,
		isFetching: isSearchTokenDataFetching,
	} = useReadContracts({
	...
	const filteredTokens = useMemo(() => {
		if (searchQuery === '') {
			...
			if (
				searchTokenData &&
				searchTokenData[0]?.status === 'success' &&
				searchTokenData[1]?.status === 'success'
			) {
		...
		availableTokens,
		searchTokenData,
		isSearchTokenDataLoading,
		isSearchTokenDataFetching,
	]);
	...
	const handleTokenSelectLocal = useCallback(
		(token: Token) => {
			onSelect(token);
						...
						isAddress(searchQuery) ? (
							...
							filteredTokens.map((token) => (
								<TokenListItem
									...
									searchTokenData={
										isAddress(searchQuery) ? searchTokenData : undefined
									}
